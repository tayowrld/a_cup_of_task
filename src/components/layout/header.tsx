...
import { IoCreate, IoReturnDownBack, IoTrophyOutline } from "react-icons/io5";
...
const Header = ({ onCreateTask, onToggleEdit }: HeaderProps) => {
        ...
        {user && (
          <div ref={menuRef} className="absolute top-3 right-4">
            <div
              ...
              className="flex items-center space-x-2 cursor-pointer bg-amber-50 p-1 rounded-full hover:bg-amber-100 transition-all duration-300"
            >
            ...
            {profileMenu && !editingProfile && (
              <div className="absolute left-[100%] mt-2 w-40 bg-white/20 backdrop-blur-lg rounded-lg shadow-lg z-50 translate-x-[-100%] cursor-pointer">
                <button
                  onClick={() => setEditingProfile(true)}
                  className="w-full text-left px-4 py-2 text-white hover:bg-white/30 transition cursor-pointer"
                >
                  Изменить профиль
                </button>
              </div>
